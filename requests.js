...
async function handleSubmit(e) {
    ...
    try {
        ...
        </svg> Subiendo archivos y enviando...`;
        ...
        formData.forEach((value, key) => {
            ...
            // Catch File objects to upload them properly
        ...
        if (filesToUpload.length > 0) {
            for (const file of filesToUpload) {
                try {
                    const fileRef = ref(storage, `statistical_requests_files/${Date.now()}_${file.name}`);
                    ...
                    alert(`Ocurrió un error al subir el archivo "${file.name}". Se enviará la solicitud sin ese archivo adjunto. (${uploadError.message})`);
                }
            }
        }
